...
# Minimizes the total distance travelled by the trucks
minimize truck_distance:
	sum{i in N, j in N, t in T:i<>j}c[i,j]*x[i,j,t] +
	sum{d in D, t in T} z[d,t] * l[d];

subject to
# Every truck route must be below the maximum truck distance,
# now with a set value from the previous run.
MaxTimeForEachTruck{t in T}:
    sum{i in N, j in N: i <> j} c[i,j] * x[i,j,t] + 
    sum{d in D} l[d] * z[d,t] <= 50.88;
