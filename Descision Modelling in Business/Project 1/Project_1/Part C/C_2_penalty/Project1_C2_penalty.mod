...
param I{M};			# demand increase for all markets
param P{M};			# penalty cost for not meeting demand in market m
...
#### Revised ojective function ####

minimize Cost: 
	sum{r in R, f in F} x[r, f] * Ctra1[r, f] +
...
DemandConstraint{m in M}: # revised demand constraint - must be less or equal to demand in marked m
	sum{f in F} y[f, m] <= D[m]*I[m]; # note the added increase parameter
...
UnsatisfiedDemand{m in M}: # 
 z[m] = (D[m] * I[m] - sum{f in F} y[f, m]); # penalty costs
